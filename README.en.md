...
- Floating captions: real-time transcription feedback near the bottom of the screen. Short text stays on one line; wrapped, newline-delimited, or longer text with clear pause punctuation prefers up to two visible lines when height allows, using the largest font size that does not clip. Captions show text, processing state, and errors only.
