...
录音和处理过程中会在当前屏幕居下显示悬浮字幕，用于实时查看转写内容与必要状态提示；短文本保持单行显示，实时文本换成多行、ASR 返回换行或较长文本存在明显停顿断点时，会在高度允许时优先显示两行，并在不裁切的前提下尽量放大字号；尺寸不足时自动退回单行滚动。字幕配色和透明度可在选项页调整，尺寸、位置和自定义颜色保留在 `config.toml`。
...
- 悬浮字幕：录音时在屏幕居下显示实时识别文本，不抢焦点；短文本保持一行，多行或带明显停顿断点的较长文本在高度允许时最多显示两行。选项页展示字幕预览、预设配色和透明度预设，自定义颜色、宽高和位置保留在 `config.toml`。
