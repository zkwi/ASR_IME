...
左侧导航按使用任务拆分为首页、热词、API配置、选项、隐私与本地数据和统计分析：热词页管理常用热词、润色提示词、上下文和自动热词，API配置页管理豆包 ASR 必填配置与可选大模型 API，选项页管理快捷键、粘贴方式、麦克风、字幕外观、开机启动、关闭行为、更新和诊断，隐私与本地数据页说明各类数据的保存位置、上传边界和清理入口。热词、API配置和选项页直接从正文分组开始，不再重复显示通用配置状态头部；可见设置按页面分组直接展示，协议地址、剪贴板快照、重试次数等底层参数仅支持通过 `config.toml` 修改。统计页展示最近 24 小时、最近 7 日、平均输入速度和按日使用情况，新识别结果写入后会刷新。
...
- 托盘常驻：关闭主窗口默认隐藏到托盘，输入和处理期间托盘图标会切换为输入中样式；右键托盘可打开配置、查看日志、问题反馈、检查更新、重启程序或退出，也可在选项页改为直接退出或每次询问。
...
- 隐私与本地数据：左侧导航和选项页均提供入口，可查看最近上下文、自动热词历史、统计、ASR 音频、屏幕 OCR、LLM 润色文本和剪贴板的保存/上传边界，并清空本地上下文、热词历史和统计数据。
...
默认不会。统计只记录时长、字数、速度等非正文数据；最近上下文和自动热词候选默认关闭。屏幕 OCR 上下文默认开启但不落盘，也不保留最近几次截图 OCR，只在本轮请求中临时发送给 ASR/LLM，可在选项页或隐私与本地数据页关闭；本地上下文、热词历史和统计可在隐私与本地数据页清空。
