...
- 实时语音识别：对接豆包 `bigmodel_async` WebSocket，默认开启二遍识别和 `full` 全量结果；字幕只作反馈，最终粘贴必须等到豆包最终包，优先使用二遍分句，必要时用最终包整段文本补齐开头或尾部。
...
屏幕 OCR 上下文默认开启，可在选项页关闭、测试或切换识别范围。默认范围是当前显示器，适合参考一个文档并在另一个窗口输入；如屏幕中有敏感内容，可切换为仅当前窗口或关闭。OCR 文本只保留在本轮请求内，不写入日志、统计、配置文件，也不会缓存最近 2-3 份截图 OCR 内容。发送前会轻量合并中文字符之间的多余空格，避免 `屏 幕 OCR 上 下 文` 这类结果干扰上下文；英文缩写、快捷键和路径间距会尽量保留。ASR 建连前默认最多等待 300ms 获取 OCR 上下文，失败或超时会跳过，不影响录音、最终识别和粘贴主链路。
...
停录时，`stop_grace_ms` 是请求的最短尾音等待时间，默认约 `200ms`。若尾音窗口内仍检测到人声，会继续等待到安静后再结束采集；默认最坏上限约 `400ms`，避免噪声导致无法结束。不足一片的尾部音频会在关闭麦克风前补发给 ASR，不再额外追加尾部静音，最后一个音频包会直接作为负包发送，帮助豆包触发二遍最终判停。
