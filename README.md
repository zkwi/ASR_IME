...
开始发送真实音频前，VoxType 只补约 `50ms` 头部静音，帮助豆包稳定起始识别，降低开头几个字被截断的概率。豆包返回的中间识别文本会尽快显示在悬浮字幕中，但最终粘贴仍必须等待豆包最终包。

停录时，`stop_grace_ms` 是请求的最短尾音等待时间；VoxType 还会使用内部下限，避免点击结束后立刻截断最后发音。若尾音窗口内仍检测到人声，会继续等待到安静后再结束采集，并有内部上限避免噪声导致无法结束；不足一片的尾部音频会在关闭麦克风前补发给 ASR，最后只追加约 `50ms` 尾部静音，并将最后一个音频包直接作为负包发送，帮助豆包触发二遍最终判停。
