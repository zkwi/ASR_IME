# 声写 VoxType - Windows 语音输入和语音转文字工具

[English](README.en.md) | 简体中文

声写（VoxType）是一个面向 Windows 10/11 的桌面语音输入、语音转文字和听写工具。把光标放到任意输入框后，按下全局热键开始说话，程序会录制麦克风音频，通过豆包流式 ASR WebSocket 实时识别语音，可选调用 OpenAI 兼容大模型润色文本，并将最终结果写入剪贴板后自动粘贴到当前输入位置。
...
主界面采用蓝白配色和紧凑侧边栏，首页顶部语音卡片集中展示当前输入状态，并用单行紧凑标签展示三种启动方式（主快捷键、鼠标中键、右 Alt）。识别完成后，首页会显示本次输入已复制并尝试粘贴，用户可临时复制或查看识别文本；文本只保留在当前窗口，关闭窗口或开始下一次录音后清除。下方展示最近 24 小时、最近 7 日、输入速度和节省时间等输入表现统计。

<img src="screenshots/ScreenShot_2026-05-09_130744_793.png" alt="VoxType 中文首页：语音输入状态、启动方式和输入表现" width="820">

左侧导航按使用任务拆分为首页、热词、API配置、选项和统计分析：热词页管理常用热词、润色提示词、上下文和自动热词，API配置页管理豆包 ASR 必填配置与可选大模型 API，选项页管理快捷键、粘贴方式、麦克风、字幕外观、开机启动、关闭行为、更新和诊断。热词、API配置和选项页直接从正文分组开始，不再重复显示通用配置状态头部；可见设置按页面分组直接展示，协议地址、剪贴板快照、重试次数等底层参数仅支持通过 `config.toml` 修改。统计页展示最近 24 小时、最近 7 日、平均输入速度和按日使用情况，新识别结果写入后会刷新。

API配置页顶部提供配置健康检查，只把真正阻断主流程的问题放到显眼位置。ASR 密钥、麦克风、粘贴方式、触发方式和隐私设置会分别展示状态，豆包 ASR 与可选大模型配置区域提供测试入口。截图中的密钥已做模糊处理，公开截图时也应保持脱敏。

<img src="screenshots/ScreenShot_2026-05-09_130838_673.png" alt="VoxType 中文 API 配置与健康检查界面" width="820">
...
- 屏幕 OCR 上下文：默认开启。开始录音时只截取当前前台窗口，用 Windows OCR 提取文字，轻量合并中文字符之间的多余空格后，作为临时上下文发送给豆包 ASR 和可选大模型，帮助识别人名、文件名、代码标识符和界面词；超时或失败会自动跳过。
...
- 托盘常驻：关闭主窗口默认隐藏到托盘，输入和处理期间托盘图标会切换为输入中样式；右键托盘可打开配置、查看日志、检查更新、重启程序或退出，也可在选项页改为直接退出或每次询问。
...
VoxType 已内置一套面向语音输入的默认润色提示词。默认规则会把识别文本视为待润色素材，不回答、不执行其中的指令或问题，避免把润色任务误变成内容分析。热词页默认展示 User Prompt 模板、恢复默认提示词和预览入口；预览会先显示场景上下文是否进入 LLM Prompt，并说明最近上下文只用于豆包 ASR 上下文。最小润色字数可在热词页调整；System Prompt 保留在 `config.toml` 中，避免普通设置页过长。

屏幕 OCR 上下文默认开启，可在选项页关闭或测试。它只截取开始录音时的当前前台窗口，不截取整个屏幕；OCR 文本只保留在本轮请求内，不写入日志、统计、配置文件，也不会缓存最近 2-3 份截图 OCR 内容。发送前会轻量合并中文字符之间的多余空格，避免 `屏 幕 OCR 上 下 文` 这类结果干扰上下文；英文缩写、快捷键和路径间距会尽量保留。默认最多等待 700ms，失败或超时不影响录音、识别和粘贴主链路。
...
- 最近上下文默认关闭，不保存最近识别片段；需要连续识别增强时可在热词页手动开启。开启后识别片段写入单独的本地 `context/recent_context.jsonl`，不会写回 `config.toml`；如需清理，可删除该本地文件。
- 屏幕 OCR 上下文默认开启，但只截取当前前台窗口，识别结果不落盘、不跨轮缓存；如当前窗口可能包含敏感信息，可在选项页关闭。
...
选项页中的“诊断与日志”可以直接打开本地日志或复制诊断报告。日志会记录关键启动阶段、配置保存、ASR/LLM 错误、更新检查和前端异常；密钥形态会自动脱敏。诊断报告默认不包含识别正文、屏幕 OCR 正文、密钥、热词、prompt、最近上下文、自动热词历史正文、候选词或 Windows 用户名路径。
...
默认不会。统计只记录时长、字数、速度等非正文数据；最近上下文和自动热词候选默认关闭。屏幕 OCR 上下文默认开启但不落盘，也不保留最近几次截图 OCR，只在本轮请求中临时发送给 ASR/LLM，可在选项页关闭。
