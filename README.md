...
左侧导航按使用任务拆分为首页、热词、API配置、选项和统计分析：热词页管理常用热词、润色提示词、上下文和自动热词，API配置页管理豆包 ASR 必填配置与可选大模型 API，选项页管理快捷键、粘贴方式、麦克风、字幕外观、开机启动、关闭行为、更新和诊断。低频设置默认展开，协议地址、剪贴板快照、重试次数等底层参数仅支持通过 `config.toml` 修改。统计页展示最近 24 小时、最近 7 日、平均输入速度和按日使用情况，新识别结果写入后会刷新。
...
- 悬浮字幕：录音时在屏幕居下显示实时识别文本，不抢焦点；选项页展示字幕预览、预设配色、透明度预设、自定义颜色、宽高和位置。
...
选项页中的“诊断与日志”可以直接打开本地日志或复制诊断报告。日志会记录关键启动阶段、配置保存、ASR/LLM 错误、更新检查和前端异常；密钥形态会自动脱敏。诊断报告默认不包含识别正文、密钥、热词、prompt、最近上下文、自动热词历史正文、候选词或 Windows 用户名路径。
...
默认不会。统计只记录时长、字数、速度等非正文数据；最近上下文和自动热词候选默认关闭。启用这些高级功能前，应确认本地正文历史的隐私影响。
