...
左侧导航按使用任务拆分为首页、热词、API配置、选项和统计分析：热词页管理常用热词、润色提示词、上下文和自动热词，API配置页管理豆包 ASR 必填配置与可选大模型 API，选项页管理快捷键、粘贴方式、麦克风、字幕外观、开机启动、关闭行为、更新和诊断。可见设置按页面分组直接展示，协议地址、剪贴板快照、重试次数等底层参数仅支持通过 `config.toml` 修改。统计页展示最近 24 小时、最近 7 日、平均输入速度和按日使用情况，新识别结果写入后会刷新。
...
- 屏幕 OCR 上下文：默认开启。开始录音时只截取当前前台窗口，用 Windows OCR 提取文字，作为临时上下文发送给豆包 ASR 和可选大模型，帮助识别人名、文件名、代码标识符和界面词；超时或失败会自动跳过。
...
屏幕 OCR 上下文默认开启，可在选项页关闭或测试。它只截取开始录音时的当前前台窗口，不截取整个屏幕；OCR 文本只保留在本轮请求内，不写入日志、统计或配置文件。默认最多等待 700ms，失败或超时不影响录音、识别和粘贴主链路。
...
- 屏幕 OCR 上下文默认开启，但只截取当前前台窗口，识别结果不落盘；如当前窗口可能包含敏感信息，可在选项页关闭。
...
默认不会。统计只记录时长、字数、速度等非正文数据；最近上下文和自动热词候选默认关闭。屏幕 OCR 上下文默认开启但不落盘，只在本轮请求中临时发送给 ASR/LLM，可在选项页关闭。
