...
阿里云模式下，VoxType 使用 Bearer API Key 建立 WebSocket 连接，发送 `run-task`、PCM 音频帧和 `finish-task`，并等待服务端 `task-finished` 后才把最终文本交给润色/粘贴链路。`workspace_id` 会自动拼接为 `wss://{WorkspaceId}.{region}.maas.aliyuncs.com/api-ws/v1/inference`；只有阿里云文档或控制台要求自定义 endpoint 时才填写 `websocket_url`。`language_hint` 默认留空使用自动识别；固定语种可填 `zh`、`en`、`ja`、`ko`、`yue`。
...
默认不会。统计只记录时长、字数、速度等非正文数据；最近上下文和自动热词候选默认关闭。最近上下文开启后会发送给当前 ASR 服务；只有额外开启“润色时参考最近上下文”并实际触发大模型润色时，才会把受限前文作为参考信息发送给大模型。屏幕 OCR 上下文默认开启但不落盘，也不保留最近几次截图 OCR，只在本轮请求中临时发送给 ASR/LLM；其中发给 LLM 的 OCR 会按预算压缩，可在选项页或隐私与本地数据页关闭；本地上下文、候选生成历史和统计可在隐私与本地数据页清空。清空按钮只删除 VoxType 本地文件，第三方 ASR/LLM 服务是否留存取决于对应服务商。
