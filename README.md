...
近期实测较稳定的组合是：保留默认 `200ms` ASR 分片，把速度优化放在首字加速、`20ms` 响应轮询、`50ms` 字幕节流和 `500ms` OCR 上下文等待上；最终文本仍只接受豆包最终包。`definite=true` 分句用于稳定最终结果，但当最终包与分句高度重合且补齐了开头或尾字时，应优先保留最终包整段文本，避免回退成缺字输出。
...
停录时，`stop_grace_ms` 是请求的最短尾音等待时间，默认约 `100ms`。若尾音窗口内仍检测到人声，会继续等待到安静后再结束采集；默认最坏上限约 `200ms`，避免噪声导致无法结束。不足一片的尾部音频会在关闭麦克风前补发给 ASR，不再额外追加尾部静音，最后一个音频包会直接作为负包发送，帮助豆包触发二遍最终判停。
