...
- 实时语音识别：对接豆包 `bigmodel_async` WebSocket，默认开启二遍识别；字幕只作反馈，最终粘贴只使用 `definite=true` 的二遍分句文本。
...
5. 录音时查看屏幕居下悬浮字幕。
6. 再按一次触发键停止录音。
7. 程序等待豆包二遍最终结果，可选润色，然后自动粘贴到当前焦点输入框。若粘贴快捷键发送失败，识别文本会保留在剪贴板，可手动 `Ctrl + V`。
