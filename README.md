...
VoxType 会把麦克风实际采集到的 PCM 音频统一转换为豆包大模型流式 ASR 支持的 `16000Hz`、单声道、16-bit PCM 后再发送；`sample_rate` 和 `channels` 只作为底层采集偏好，普通用户不需要修改。

开始发送真实音频前，VoxType 会先补一小段头部静音，帮助豆包稳定起始识别，降低开头几个字被截断的概率。

停录时，`stop_grace_ms` 是请求的最短尾音等待时间；VoxType 还会使用内部下限，避免点击结束后立刻截断最后发音。若尾音窗口内仍检测到人声，会继续等待到安静后再结束采集，并有内部上限避免噪声导致无法结束；不足一片的尾部音频会在关闭麦克风前补发给 ASR，发送结束包前还会追加短静音，帮助豆包触发二遍最终判停。
