...
4. 用户停止录音后，先停止采集流，再 flush `PcmSink`：
   ...
   - flush 不足一个音频包的分片尾部。
5. ASR WebSocket 将约 50ms 头部静音并入第一包真实音频，第一包约 150ms，后续默认 200ms，帮助服务端稳定识别开头且避免独立 50ms 小包。
...
- 第一包约 150ms，后续真实音频包默认 200ms；不足一片的尾包按实际音频时长安排发送，不再按默认 200ms 人为拉长。
...
cargo test initial_audio_padding_is_merged_into_first_real_packet
