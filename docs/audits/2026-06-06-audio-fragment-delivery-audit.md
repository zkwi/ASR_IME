...
当前维护入口：[ASR 质量与延迟守门清单](../asr-quality-latency-guardrails.md)。后续改 ASR 速度、识别效果、OCR 等待、尾音或最终包选择时，应以该清单为主，本审计保留当时的边界分析。
...
- 0.1.102 后实测较稳定的组合是：默认 200ms ASR 音频包不动，速度主要来自 `enable_accelerate_text`、20ms WebSocket 响应轮询、50ms 字幕节流和 500ms OCR 等待上限；不要为了体感速度把默认音频分片改成更短。
- 识别效果优先时，`enable_ddc=false` 比语义顺滑更稳，尤其是专有词、短命令、代码标识符和标点敏感口述；如果已有用户配置显式开启，应尊重用户选择。
- 尾字问题分两类处理：若最终包已补齐尾字但 `definite=true` 分句较短或前文略有改写，最终文本选择应保留高度重合且未明显残缺的最终包；若真实音频被切断，则应调大停录尾音窗口或修 flush/负包链路，不能用中间文本兜底。
...
- 屏幕 OCR 上下文默认只等待 500ms，超时正常跳过，在近期实测命中率和 ASR 建连等待之间折中。
- 负包后的最终结果等待使用正常响应轮询节奏，减少停录后的本地 CPU 抖动。
- ASR 最终文本必须来自最终包，并优先使用最终包里的 `result.text`；只有中间文本或 definitive 分句但没有最终包时会失败。
- 最终包补齐 definitive 分句缺失的尾字、且与 definitive 分句高度重合时，优先使用最终包文本。
