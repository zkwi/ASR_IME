...
The recently verified stable combination is to keep the default `200ms` ASR packet size and put speed work into first-word acceleration, `20ms` response polling, `50ms` caption throttling, and a `500ms` OCR-context wait. Final output still accepts only Doubao's final package. `definite=true` utterances stabilize the final result, but when the final package highly overlaps those utterances and recovers missing head or tail words, VoxType should keep the final full text.
