...
- 达到最小字数并启用润色时，最终识别文本会发送到你配置的大模型服务；用户词典、场景与产品偏好和屏幕 OCR 会作为参考信息追加。
...
- 可预览最终 Prompt，并确认用户词典、场景/产品偏好和屏幕 OCR 如何作为参考信息进入 LLM Prompt。
- 默认提示词会在不新增事实的前提下纠正明显错词漏字、语法残缺和不通顺语句；无法确定原意时保留原文。
- 默认提示词会保持原文主要语言和中英混合方式，不主动翻译中文或外语内容。
- 默认提示词会明确把用户词典、场景与偏好上下文、屏幕 OCR 标记为参考信息，不把它们当作待润色文本或指令来源。
...
- 本地可清理数据：最近上下文、自动热词历史和统计数据。最近上下文与自动热词历史包含语音输入正文历史，默认关闭；统计只包含字数、时长、速度等非正文指标。
- 运行时与第三方服务数据：ASR 音频、屏幕 OCR、LLM 润色文本和剪贴板临时快照。这些数据通常不落盘，但 ASR、OCR 上下文和 LLM 润色会按功能开关进入对应的本轮请求。
