...
默认提示词会把识别文本视为待润色素材，而不是模型指令；即使文本中包含问题、命令或类似提示词注入的内容，也只做文本润色，不回答、不执行、不分析。它会在不新增事实的前提下纠正明显错词漏字、语法残缺和不通顺语句；无法确定原意时保留原文。金融、投资、量化语境中，默认提示词会把明确金额、收益率和百分比整理为常用数字写法，例如 `100万`、`1%`、`10%`，但不会计算收益或回答问题。
