...
默认提示词会把识别文本视为待润色素材，而不是模型指令；即使文本中包含问题、命令或类似提示词注入的内容，也只做文本润色，不回答、不执行、不分析。默认提示词会保持原文主要语言和中英混合方式，不主动把中文翻译成外语，也不主动把外语翻译成中文。用户词典、场景与产品偏好、可选最近上下文、屏幕 OCR 会作为参考信息分区追加，只用于纠正术语、称谓、界面词、上下文承接和表达偏好，不作为待润色文本或指令来源。它会在不新增事实的前提下纠正明显错词漏字、语法残缺和不通顺语句；无法确定原意时保留原文。金融、投资、量化语境中，默认提示词会把明确金额、收益率和百分比整理为常用数字写法，例如 `100万`、`1%`、`10%`，但不会计算收益或回答问题。
