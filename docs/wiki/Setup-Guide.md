...
屏幕 OCR 上下文默认开启。它只截取开始录音时的当前前台窗口，不截取整个屏幕；识别结果只作为本轮 ASR 和可选大模型请求的临时上下文，不写入日志、统计或配置。当前窗口含敏感信息时，可在选项页关闭。
...
| 屏幕 OCR 上下文 | 开启 | 只识别当前窗口文字，提升专有名词和界面词命中率；敏感窗口可关闭 |
| 最近上下文 | 关闭 | 默认更保守 |
| 自动热词候选 | 关闭 | 默认不保存正文历史 |
...
屏幕 OCR 上下文默认开启，但不会保存正文历史；它只在开始录音时识别当前前台窗口，并把结果临时附加给本轮 ASR/LLM 请求。若窗口含敏感内容，可以在选项页关闭。
