...
VoxType 会把 ASR 实际发送分片限制在豆包建议的 `100-200ms`，默认 `200ms`。第一片真实音频前会补一小段头部静音，帮助豆包稳定识别开头几个字。`stop_grace_ms` 表示停录后请求的最短尾音等待时间；VoxType 还会使用内部下限，避免点击结束后立刻截断最后发音。若尾音窗口内仍检测到人声，会继续等到安静后再结束采集，并有内部上限避免噪声导致无法结束；不足一片的尾部音频会在关闭麦克风前补发给 ASR，最后一个音频包会直接作为负包发送，帮助豆包触发二遍最终判停。
