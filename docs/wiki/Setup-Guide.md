...
最小润色触发长度位于热词与提示词页，支持 0 到 10000；中文按单字计，英文和数字按连续词片段计。System Prompt 保留在 `config.toml`，避免普通设置页过长。
...
Resource ID、ASR WebSocket 地址、模型名、最终结果等待超时、最长录音秒数、尾音保留毫秒、LLM 超时、主热键启用开关、录音时静音系统音量、屏幕 OCR 字数上限和等待时间、字幕自定义尺寸/位置/颜色、剪贴板恢复延迟、快照大小和重试参数属于底层参数，保留在 `config.toml`。LLM 最小润色触发长度和参考信息预算可在热词与提示词页调整。
