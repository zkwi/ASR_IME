...
| 隐私与本地数据 | 查看保存位置、上传边界并清空本地上下文、热词历史和统计 |
...
隐私与本地数据页会把最近上下文、自动热词历史、统计数据、ASR 音频、屏幕 OCR、LLM 润色文本和剪贴板快照的保存/上传边界集中展示，也提供清空本地上下文、热词历史和统计数据的入口。
...
托盘右键菜单可打开配置、查看日志、问题反馈、检查更新、重启程序或退出。配置更新后需要重新加载程序状态时，可直接使用“重启程序”。
