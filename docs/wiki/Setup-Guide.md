...
麦克风声音偏小但清晰时，可以在录音排障中设置 `input_gain_db`，由客户端在发送 ASR 前放大 16-bit PCM；建议先用“校准增益”朗读几句话，VoxType 只在本地分析 RMS 与峰值并给出推荐 dB，不保存或上传音频；也可以手动先试 `+6 dB`，仍偏小时再试 `+12 dB`。每轮录音结束后，首页会显示一张轻量的录音质量卡片，展示最近一轮的 RMS、峰值、有效语音比例和建议；这些指标不包含正文，也不会写入主统计表。
