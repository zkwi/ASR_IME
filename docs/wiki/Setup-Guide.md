...
- 默认展开：普通用户必须知道、经常修改和偶尔排障会用到的设置。
- 可收起的低频设置：场景上下文、备用触发、字幕精调、录音声音等。
...
最小润色字数位于完整设置；System Prompt 保留在 `config.toml`，避免普通设置页过长。
...
- 录音与排障：连续低音量自动停止、录音时静音系统音量。

ASR WebSocket 地址、模型名、最长录音秒数、剪贴板恢复延迟、快照大小和重试参数属于底层参数，保留在 `config.toml`。
