...
1. 开始录音时，`SessionController` 加载配置，启动麦克风采集，并按需启动当前前台窗口 OCR。
2. `screen_context.rs` 只截取当前前台窗口，OCR 文本只在本轮请求内使用；失败或超时会跳过，不阻断录音和粘贴。
3. `asr.rs` 组装豆包 ASR 请求。热词、最近上下文、场景上下文和 OCR 结果会作为上下文发送；OCR 会标注为“当前窗口 OCR 上下文”，不是用户指令或待识别文本。
4. `asr_ws.rs` 维护流式 WebSocket 会话，实时片段用于悬浮字幕，最终结果进入后处理。
5. `llm_post_edit.rs` 只在 LLM 已启用、文本长度达到 `min_chars` 且 Base URL、API Key、模型名完整时调用；否则直接使用 ASR 最终文本。
