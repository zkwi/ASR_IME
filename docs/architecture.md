...
5. `llm_post_edit.rs` 只在 LLM 已启用、文本长度达到 `min_chars` 且 Base URL、API Key、模型名完整时调用；用户词典、场景与偏好上下文、屏幕 OCR 会作为参考信息分区追加，并明确不是待润色文本或指令来源。默认提示词会保持待润色文本的主要语言，不主动翻译中文或外语内容；否则直接使用 ASR 最终文本。
