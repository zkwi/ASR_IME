...
润色规则：
- 短文本只做轻量润色：修正明显 ASR 错误、错词漏字、标点、断句和术语；不扩写、不重排。
- 长文本可在不新增事实、不改变意图的前提下做必要改进：删除口水词、无意义语气词、重复表达、无效停顿和修正过程，合并同义反复，按最终意图调整顺序、分段或分点，让条理更清晰、逻辑更明确。
- 遇到口述改口时，例如先说 A 又改成 B，直接采用 B；删除 A 和修正连接词，不保留改口过程；无法判断最终意图时保留原文。
- 原文包含多个要点、明显层级，或用户明确要求分点/分段时，可分段、分行或分点；否则保持自然段落。
...
fn default_user_prompt_template() -> String {
    r#"请润色下面的 ASR 文本：短文本轻量纠错；长文本删除口水词、重复和自我修正过程，不要保留或改写成“准确说/更准确地说/应该是”；不要主动添加 Markdown 或反引号。只输出最终文本：
...
mod tests {
    ...
    fn default_llm_prompts_are_ready_for_voice_input() {
        ...
        assert!(config
            ...
            .system_prompt
            .contains("短文本只做轻量润色"));
        assert!(config.llm_post_edit.system_prompt.contains("长文本可"));
        assert!(config.llm_post_edit.system_prompt.contains("无意义语气词"));
        assert!(config.llm_post_edit.system_prompt.contains("最终意图"));
        assert!(config
            .llm_post_edit
            .system_prompt
            .contains("先说 A 又改成 B"));
